    ...
    @if (loading && addresses.length === 0) {
      <div class="space-y-3">
        @for (i of [1,2]; track i) {
          <div class="bg-white rounded-xl p-5 space-y-3 theme-border border">
    ...
    @if (!loading || addresses.length > 0) {
      <div class="space-y-3 mb-4">
        @for (addr of addresses; track addr.id) {
          <div class="bg-white rounded-xl p-4 border transition-all duration-200"
               [class]="addr.isDefault ? 'border-gold/40' : 'theme-border'">
            <div class="flex items-start justify-between gap-3">
              <div class="flex items-start gap-3 min-w-0">
                <div class="w-9 h-9 rounded-lg flex items-center justify-center flex-shrink-0 mt-0.5"
                     [class]="addr.isDefault ? 'bg-gold/8 border border-gold/20' : 'bg-gray-50 border theme-border'">
                  <svg class="w-4 h-4" [class.text-gold]="addr.isDefault" [class.theme-muted]="!addr.isDefault" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <div class="min-w-0">
                  <div class="flex items-center gap-2 flex-wrap">
                    ...
                    @if (addr.isDefault) {
                      <span class="px-2 py-0.5 rounded-full text-[10px] font-semibold" style="color:#a07010; background:rgba(212,175,55,0.08); border:1px solid rgba(212,175,55,0.15);">Par défaut</span>
                    }
                  </div>
                  <p class="text-sm theme-muted mt-0.5">{{ addr.prenom }} {{ addr.nom }}</p>
                  <p class="text-sm theme-muted">{{ addr.quartier }}, {{ addr.ville }}</p>
                  <p class="text-xs theme-muted mt-0.5 opacity-70">{{ addr.phone }}</p>
                  @if (addr.complement) { <p class="text-xs theme-muted opacity-70">{{ addr.complement }}</p> }
                </div>
              </div>
              <div class="flex items-center gap-1 flex-shrink-0">
                @if (!addr.isDefault) {
                  <button (click)="setDefault.emit(addr.id)" title="Définir par défaut"
                    class="w-8 h-8 rounded-lg flex items-center justify-center transition-all theme-muted hover:text-gold hover:bg-gold/8">
                ...
                <button (click)="startDelete(addr.id)" title="Supprimer"
                  class="w-8 h-8 rounded-lg flex items-center justify-center transition-all hover:bg-red-50 theme-muted hover:text-red-500">
                  <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        ...
        @if (addresses.length === 0 && !loading) {
          <div class="bg-white rounded-xl p-8 text-center theme-border border">
            <div class="w-12 h-12 rounded-xl mx-auto mb-3 flex items-center justify-center bg-gray-50 border theme-border">
              <svg class="w-6 h-6 theme-muted" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.244-4.243a8 8 0 1111.314 0z"/>
              </svg>
            </div>
            <p class="text-sm font-semibold theme-text">Aucune adresse enregistrée</p>
            <p class="text-xs theme-muted mt-1">Ajoutez une adresse pour accélérer vos commandes</p>
          </div>
        }
      </div>
      ...
      @if (addresses.length < MAX_ADDRESSES && !showForm) {
        <button (click)="openForm()"
          class="w-full flex items-center justify-center gap-2 py-3 rounded-xl text-sm font-semibold transition-all duration-200 border-2 border-dashed theme-border theme-muted hover:text-gold hover:border-gold/40">
          <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      @if (showForm) {
        <div class="bg-white rounded-xl border theme-border overflow-hidden mt-4">
          ...
          <form [formGroup]="addressForm" (ngSubmit)="save()" class="p-5 space-y-4">
            <div>
              <label class="block text-xs font-semibold theme-muted uppercase tracking-wider mb-2">Type d'adresse</label>
              <div class="flex gap-2">
                @for (lbl of LABELS; track lbl) {
                  <button type="button" (click)="addressForm.patchValue({label: lbl})"
                    ...
                    [class]="addressForm.get('label')?.value === lbl ? 'text-black' : 'theme-muted hover:text-gold'"
                    [style]="addressForm.get('label')?.value === lbl ? 'background:linear-gradient(135deg, #f5cc42, #d4af37);' : 'background:rgba(0,0,0,0.03); border:1px solid rgba(0,0,0,0.08);'">
                    {{ lbl }}
                  </button>
                }
              </div>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="block text-xs font-semibold theme-muted uppercase tracking-wider mb-1.5">Prénom</label>
                <input formControlName="prenom" type="text" placeholder="Moussa"
                  class="w-full px-4 py-2.5 rounded-lg border theme-border bg-white text-sm theme-text focus:outline-none focus:border-yellow-500/60 focus:ring-2 focus:ring-yellow-500/20 transition-colors">
              </div>
              <div>
                <label class="block text-xs font-semibold theme-muted uppercase tracking-wider mb-1.5">Nom</label>
            ...
            <div>
              <label class="block text-xs font-semibold theme-muted uppercase tracking-wider mb-1.5">Téléphone</label>
              <input formControlName="phone" type="tel" placeholder="0700000000"
                class="w-full px-4 py-2.5 rounded-lg border theme-border bg-white text-sm theme-text focus:outline-none focus:border-yellow-500/60 focus:ring-2 focus:ring-yellow-500/20 transition-colors">
            </div>

            <div>
              <label class="block text-xs font-semibold theme-muted uppercase tracking-wider mb-1.5">Quartier / Rue</label>
            ...
            <div class="grid grid-cols-2 gap-3">
              <div>
                <label class="block text-xs font-semibold theme-muted uppercase tracking-wider mb-1.5">Ville</label>
                <input formControlName="ville" type="text" placeholder="Abidjan"
                  class="w-full px-4 py-2.5 rounded-lg border theme-border bg-white text-sm theme-text focus:outline-none focus:border-yellow-500/60 focus:ring-2 focus:ring-yellow-500/20 transition-colors">
              </div>
              <div>
                <label class="block text-xs font-semibold theme-muted uppercase tracking-wider mb-1.5">Pays</label>
            ...
            <div>
              <label class="block text-xs font-semibold theme-muted uppercase tracking-wider mb-1.5">Complément <span class="theme-muted normal-case font-normal">(optionnel)</span></label>
            ...
            <label class="flex items-center gap-3 cursor-pointer group">
              ...
              <div class="w-5 h-5 rounded-lg flex items-center justify-center flex-shrink-0 transition-all border-2"
                   [class]="addressForm.get('isDefault')?.value ? 'border-gold bg-gold' : 'theme-border'"
                   style="background: linear-gradient(135deg, #f5cc42, #d4af37); border-color: #d4af37;">
              ...
              <span class="text-sm font-medium theme-muted group-hover:text-gold transition-colors">Définir comme adresse par défaut</span>
            </label>

            <div class="flex items-center justify-end gap-3 pt-2">
              <button type="button" (click)="closeForm()" class="px-5 py-2.5 rounded-lg text-sm font-semibold theme-muted hover:opacity-80 transition-colors">Annuler</button>
    ...
    @if (deleteId) {
      <div class="fixed inset-0 z-[70] flex items-center justify-center p-4">
        ...
        <div class="relative w-full max-w-sm bg-white rounded-2xl shadow-xl p-6">
          ...
          <h3 class="text-base font-bold theme-text text-center mb-1">Supprimer cette adresse ?</h3>
          <p class="text-sm theme-muted text-center mb-4">Cette action est irréversible.</p>
          <div class="flex gap-3">
            <button (click)="deleteId = null" class="flex-1 py-2.5 rounded-xl text-sm font-semibold transition-colors" style="border:1px solid rgba(0,0,0,0.08); color:#6b7280; hover:background:#f8f5f0;">Annuler</button>
